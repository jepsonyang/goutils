...
func RedisStringSet(conn redis.Conn, redisType RedisType, value string, expire int) error {
	...
	return RedisStringSetByKey(conn, redisType.GetKey(), value, expire)
}

func RedisStringSetByKey(conn redis.Conn, key string, value string, expire int) error {
	args := redis.Args{}
	args = args.Add(key)
	args = args.Add(value)
	if expire >= 0 {
...
func RedisStringGet(conn redis.Conn, redisType RedisType) (string, error) {
	...
	return RedisStringGetByKey(conn, redisType.GetKey())
}

func RedisStringGetByKey(conn redis.Conn, key string) (string, error) {
